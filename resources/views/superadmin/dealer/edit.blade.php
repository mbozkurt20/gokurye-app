...
@section('content')
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div>
                <h1 class="text-2xl font-black tracking-tighter text-slate-800 uppercase leading-none m-0">
                    PARTNER <span class="text-slate-400">VERİLERİNİ DÜZENLE</span>
                </h1>
                <p class="text-[11px] font-bold text-slate-400 uppercase tracking-widest mt-2">
                    Mevcut Partner: <span class="text-indigo-600">{{ $dealer->name }}</span>
                </p>
            </div>
                ...
                <div class="col-xl-7">
                    <div class="bg-white !rounded-[40px] shadow-sm border border-slate-50 overflow-hidden mb-4">
                        <div class="p-8 border-b border-slate-50 bg-slate-50/30">
                            <h4 class="text-xs font-black text-slate-800 uppercase tracking-[0.2em] m-0">HESAP VE İLETİŞİM AYARLARI</h4>
                        </div>
                        <div class="p-8">
                            <div class="row g-4">
                                <div class="col-md-6">
                                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">PARTNER ADI</label>
                                    <input required type="text" class="form-control !rounded-2xl border-slate-100 bg-slate-50/50 p-4 font-bold text-slate-700 focus:ring-2 focus:ring-indigo-100 transition-all" name="name" value="{{ $dealer->name }}">
                                </div>
                                <div class="col-md-6">
                                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">E-POSTA</label>
                                    <input required type="email" class="form-control !rounded-2xl border-slate-100 bg-slate-50/50 p-4 font-bold text-slate-700 focus:ring-2 focus:ring-indigo-100 transition-all" name="email" value="{{ $dealer->email }}">
                                </div>
                                <div class="col-md-6">
                                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">TELEFON</label>
                                    <input required type="tel" class="form-control !rounded-2xl border-slate-100 bg-slate-50/50 p-4 font-bold text-slate-700 focus:ring-2 focus:ring-indigo-100 transition-all" name="phone" value="{{ $dealer->phone }}">
                                </div>
                                <div class="col-md-6">
                                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">YENİ PAROLA (BOŞ BIRAKILABİLİR)</label>
                                    <input type="password" class="form-control !rounded-2xl border-slate-100 bg-slate-50/50 p-4 font-bold text-slate-700 focus:ring-2 focus:ring-indigo-100 transition-all" name="password" placeholder="••••••••">
                                </div>
                                <div class="col-md-6">
                                    ...
                                    <select required class="form-control select2" name="city_id" id="city-select">
                                        ...
                                        @foreach($cities as $city)
                                            <option {{$dealer->city_id == $city->id ? 'selected' : ''}} value="{{$city->id}}" data-lat="{{$city->lat}}" data-lng="{{$city->lng}}">{{$city->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-6">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="number" step="0.01" min="0" max="100" class="form-control !rounded-2xl border-slate-100 bg-slate-50/50 p-4 font-bold text-slate-700 focus:ring-2 focus:ring-indigo-100 transition-all" name="commission_rate" value="{{ $dealer->commission_rate ?? 20 }}">
                                    <p class="text-[9px] text-slate-400 mt-1 ml-1">Her kontör yüklemesinden alınacak komisyon yüzdesi</p>
                                </div>
                                <div class="col-md-6">
                                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">TOPLAM KAZANÇ (BİLGİ)</label>
                                    <div class="form-control !rounded-2xl border-slate-100 bg-emerald-50 p-4 font-black text-emerald-700">
                                        {{ number_format($dealer->commission_balance ?? 0, 2, ',', '.') }} ₺
                                    </div>
                                </div>
                                <div class="col-12">
                                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">ADRES DETAYI (OPSİYONEL)</label>
                                    <textarea rows="4" name="address" class="form-control !rounded-2xl border-slate-100 bg-slate-50/50 p-4 font-bold text-slate-700 focus:ring-2 focus:ring-indigo-100 transition-all" placeholder="Detaylı adres...">{{ $dealer->address }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-xl-5">
                    <div class="bg-white !rounded-[40px] shadow-sm border border-slate-50 overflow-hidden h-full">
                        <div class="p-8 border-b border-slate-50 bg-slate-50/30 flex justify-between items-center">
                            <h4 class="text-xs font-black text-slate-800 uppercase tracking-[0.2em] m-0">LOKASYON YÖNETİMİ</h4>
                        ...
                        <div class="p-8">
                            ...
                            <div class="row g-3 mb-8">
                                <div class="col-6">
                                    ...
                                    <input required type="text" class="form-control !rounded-xl bg-slate-100 border-0 p-3 text-xs font-black text-slate-600" id="latit" name="lat" value="{{$dealer->latitude}}" readonly>
                                </div>
                                <div class="col-6">
                                    <label class="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1 block">LONGITUDE</label>
                                    <input required type="text" class="form-control !rounded-xl bg-slate-100 border-0 p-3 text-xs font-black text-slate-600" id="longi" name="lng" value="{{$dealer->longitude}}" readonly>
                                </div>
                            </div>
                            ...
                            <p class="text-center text-[9px] font-bold text-slate-300 uppercase mt-4 tracking-widest italic">
                                Kayıt Tarihi: {{ $dealer->created_at->format('d.m.Y H:i') }}
                            </p>
                        </div>
                    </div>
                </div>
    ...
    <style>
        ...
        .leaflet-container { font-family: 'Inter', sans-serif; border-radius: 20px; }
    </style>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#city-select').on('change', function (e, isInitial = false) {
                var cityId = $(this).val();
                var selectedOption = $(this).find('option:selected');
                var lat = selectedOption.data('lat');
                var lng = selectedOption.data('lng');

                // Eğer ilk yükleme değilse haritayı şehre kaydır
                if (!isInitial && lat && lng) {
                    map.setView([lat, lng], 13);
                    updateMarker(lat, lng);
                }

                if (cityId) {
                    let dealerDistrictId = {{ $dealer->district_id ?? 'null' }};
                    $.ajax({
            ...
            // Sayfa yüklendiğinde mevcut ilçeleri çek ama haritayı mevcut konumda tut
            $('#city-select').trigger('change', [true]);
        });
    </script>
@endsection
