...
<header class="h-20 bg-white border-b border-slate-100 sticky top-0 z-40 px-6 flex items-center justify-between shadow-sm shadow-slate-200/50">
    ...
    <div class="flex items-center gap-6">
        <a href="{{ url('/admin') }}" class="flex items-center gap-3 group transition-transform hover:scale-105">
            ...
            <div class="hidden md:block">
                ...
                <span class="text-[10px] font-bold text-brand uppercase tracking-widest">Yönetim Paneli</span>
            </div>
        </a>
    </div>
    ...
    <div class="flex items-center gap-4 lg:gap-8">
        ...
        <div class="hidden lg:flex items-center gap-4 border-r border-slate-100 pr-6">
            <div class="text-right">
                <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Davet Kodunuz</p>
                <p class="text-xs font-black text-emerald-600 tracking-wider">
                    [ {{ auth()->guard('admin')->user()->code }} ]
                </p>
            </div>
            <div class="text-right">
                <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Kontör Bakiyesi</p>
                <p class="text-xs font-black text-brand tracking-wider">
                    {{ auth()->guard('admin')->user()->top_up_balance }}
                </p>
            </div>
        </div>
        ...
        <div class="relative">
            ...
            <div id="notificationMenu" class="absolute right-0 mt-3 w-80 md:w-96 opacity-0 invisible transition-all duration-300 transform translate-y-2 z-[99999]">
                <div class="bg-white rounded-3xl shadow-2xl shadow-brand/20 border border-slate-100 overflow-hidden">
                    <div class="p-4 bg-brand flex justify-between align-items-center">
        ...
        <div class="relative">
            <button type="button" onclick="toggleProfileMenu(event)" class="flex items-center gap-3 p-1.5 pr-4 rounded-2xl hover:bg-slate-50 transition-all border border-transparent hover:border-slate-100 bg-transparent border-0 outline-none cursor-pointer">
                ...
                <div class="hidden sm:block text-left">
                    <p class="text-[11px] font-black text-slate-800 uppercase leading-none m-0">Yönetici</p>
            ...
            <div id="premiumProfileMenu" class="absolute right-0 mt-3 w-64 opacity-0 invisible transition-all duration-300 transform translate-y-2 z-[99999]">
                <div class="bg-white rounded-3xl shadow-2xl shadow-brand/20 border border-slate-100 overflow-hidden p-2">
                    <div class="p-4 bg-brand/5 rounded-2xl mb-2">
                        ...
                        <p class="text-xs font-bold text-slate-800 truncate m-0">{{ auth()->guard('admin')->user()->email }}</p>
                    </div>

                    <div class="space-y-1">
                        <a href="{{ route('admin.profile') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-brand/5 text-slate-600 hover:text-brand transition-all text-xs font-bold uppercase tracking-tight no-underline">
                            <i class="fa-solid fa-circle-user text-slate-300 w-5"></i> Profilim
                        </a>
                        <hr class="mx-4 border-slate-50 my-1">
                        <a href="{{ route('admin.logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form').submit();" class="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-rose-50 text-rose-500 transition-all text-xs font-black uppercase tracking-tight no-underline">
...
<script>
    ...
    window.onclick = function(event) {
        ['notificationMenu', 'premiumProfileMenu'].forEach(id => {
            ...
            if (!menu.classList.contains('invisible') && !menu.contains(event.target)) {
    ...
    function updateCount(change) {
        ...
        if (current > 0) {
            if (countElem.length === 0) {
                $('[onclick="toggleNotificationMenu(event)"]').append(`<span id="notificationCount" class="absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-rose-500 text-[10px] font-bold text-white border-2 border-white">${current}</span>`);
            } else {
                countElem.text(current);
            }
            $('.clear-all-container').html('<button onclick="clearAllNotifications()" class="text-[10px] font-bold text-white/80 hover:text-white uppercase tracking-tighter bg-transparent border-0 cursor-pointer">Tümünü Temizle</button>');
        } else {
            countElem.remove();
    ...
    function clearAllNotifications() {
        $.get('/admin/notifications/clear-all', function () {
    ...
    function deleteNotification(id) {
        $.get('/admin/notifications/' + id, function () {
            $(`li[data-id="${id}"]`).remove();
            updateCount(-1);
            if ($('#notificationList li').length === 0) clearAllNotifications();
        });
    }
</script>
