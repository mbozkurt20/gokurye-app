...
@section('content')
    <div class="container-fluid">
        <div class="mb-sm-4 d-flex flex-wrap align-items-center text-head">
            <h2 class="mb-3 me-auto">{{$print->name}}</h2>
            <div>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="/restaurant/prints">{{$print->name}}</a></li>
            ...
            <div class="col-xl-8 col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">Yazıcı Düzenle Formu</h4>
                    </div>
                    <div class="card-body">
                        <div class="basic-form">
                            <form method="post" action="{{route('restaurant.prints.update')}}">
                                @csrf
                                <div class="row">
                                    <input type="hidden" name="id" value="{{$print->id}}">
                                    <div class="mb-3 col-md-4">
                                        <label class="form-label">Yazıcı Adı</label>
                                        <input value="{{$print->name}}" required type="text" class="form-control" name="name" placeholder="Yazıcı Adı">
                                    </div>
                                    <div class="mb-3 col-md-4">
                                        <label class="form-label">Yazıcı Açıklması (opsiyonel)</label>
                                        <input value="{{$print->description}}" type="text" class="form-control" name="description" placeholder="Yazıcı Açıklaması">
                                    </div>
                                </div>

                                <button type="submit" class="special-button float-end mt-4">Yazıcı Güncelle</button>
                            </form>
                        </div>
                    </div>
                </div>
